...
import { filterProductsAction, Product } from "@/data/product";
...
const FeaturedProductsCarousel: React.FC = () => {
  ...
  // 🧠 Fetch products with tag="dhh" from server action
  useEffect(() => {
    (async () => {
      try {
        const { products } = await filterProductsAction({ tags: "dhh" });
        setProducts(products);
      } catch (err) {
        console.error("Failed to load DHH products", err);
      } finally {
        setIsLoading(false);
      }
    })();
  ...
  const goNext = () =>
    goToSlide(
      (currentIndex + 1) % (products ? products.length : skeletonCount)
    );
  const goPrev = () =>
    goToSlide(
      (currentIndex - 1 + (products ? products.length : skeletonCount)) %
        (products ? products.length : skeletonCount)
    );
      ...
      >
        ...
        {isLoading &&
          Array.from({ length: skeletonCount }).map((_, index) => (
            <div
              key={index}
              className="relative flex-shrink-0 w-full h-full snap-start bg-white flex flex-col md:flex-row"
            >
              <Skeleton className="w-full md:w-1/2 h-64 sm:h-80 md:h-auto" />
              <div className="w-full md:w-1/2 flex flex-col items-center justify-center gap-4 p-10">
        ...
        {/* ✅ Show fetched DHH products */}
        {!isLoading &&
          products?.map((product, index) => {
            const background = BASE_URL+product.images.find(
              (img) => img.tag === "bg"
            )?.url;
            const frontImage = BASE_URL+product.images.find(
              (img) => img.tag === "front"
            )?.url;
              ...
              >
                <div className="flex flex-col md:flex-row w-full h-full">
                  <div className="w-full md:w-1/2 h-56 sm:h-72 md:h-auto overflow-hidden">
                    <img
                      src={background || "/placeholder-bg.png"}
                      alt={product.name}
                  ...
                  <div className="w-full md:w-1/2 flex items-center justify-center bg-white py-4 sm:p-2 md:p-10">
                    <div className="relative w-full text-center">
                      ...
                      >
                        ...
                        >
                          <img
                            src={frontImage || "/placeholder-front.png"}
                            alt={product.name}
            ...
            >
              <span
                className={`block w-2 h-2 rounded-full transition-all duration-300 ${
                  currentIndex === index
                    ? "bg-gray-800 opacity-100 scale-125"
